...
for (const width of [390, 1000, 1440]) {
  test(`endpoint icons leave no gaps between grid rows at ${width}px`, async ({ page }) => {
    ...
    const gaps = await page.locator('.board-row').evaluateAll((rows) =>
      rows.map((row) => {
        const cell = [...row.children].find(
          (cell) => !cell.classList.contains('start') && !cell.classList.contains('finish'),
        );
        return row.getBoundingClientRect().height - cell.getBoundingClientRect().height;
      }),
    );
