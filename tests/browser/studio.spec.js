...
test("drag endpoints, handle unreachable routes, and cancel an active search", async ({
  page,
}) => {
  await page.goto("/");
  ...
  for (const cell of [399, 401, 361, 439])
...
test("guide traps focus and returns it to its trigger", async ({ page }) => {
  ...
  await expect(
    page.getByRole("button", { name: /Let’s find a path/ }),
  ).toBeFocused();
...
test("draw, erase, move endpoints, generate and reset", async ({ page }) => {
  await page.goto("/");
  await expect(
    page.getByRole("heading", { name: /Every path starts/ }),
  ).toBeVisible();
  ...
  await page
    .getByRole("button", { name: "Generate maze", exact: true })
    .click();
...
test("search can pause, step, resume, finish and clear", async ({ page }) => {
  ...
  await expect(page.locator(".progress-track > div")).toHaveAttribute(
    "style",
    /100%/,
  );
